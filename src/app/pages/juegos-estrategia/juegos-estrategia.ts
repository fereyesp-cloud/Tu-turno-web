...
/**
 * Componente para los juegos de estrategia
...
export class JuegosEstrategia implements OnInit {
  ...
  /** Lista de juegos de estrategia cargados desde la API */
  juegos: any[] = [];
  ...
  /**
   * Inicialización — carga los juegos de estrategia desde la API
   */
  ngOnInit() {
    this.productoService.getPorCategoria('Juegos de Estrategia').subscribe({
      next: (data) => {
        this.juegos = data;
        this.cargando = false;
  ...
  /**
   * Agrega un producto al carrito desde la página de juegos de estrategia
